...
function HookServer(listen_port, endpointDao, subscriptionDao, log) {
    assert.number(listen_port, "listen_port");
    assert.object(endpointDao, "endpointDao");
    assert.object(subscriptionDao, "subscriptionDao");
    ...
    function handlePublish(req, res, next) {
        ...
        if(req.params.hasOwnProperty(1)) {
            // slice off the preceding fwd slash
            data.extra_path = req.params[1].substr(1);
        }
